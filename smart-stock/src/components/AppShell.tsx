...
const AppShell = ({ children }: AppShellProps) => {
  ...
  const { role, logout, getAuthHeader } = useAuth();
  ...
  const activeItems = role === 'staff' ? [
    { icon: LayoutDashboard, label: "Dashboard", to: "/staff" },
    { icon: BarChart3, label: "Sales", to: "/staff/sales" },
    { icon: Package, label: "Products", to: "/products" },
    { icon: AlertTriangle, label: "Alerts Matrix", to: "/alerts" }
  ] : role === 'admin' ? [
    { icon: LayoutDashboard, label: "Nexus Command", to: "/dashboard" },
    { icon: Package, label: "Categories", to: "/admin/categories" },
    { icon: BarChart3, label: "Sales", to: "/admin/sales" },
    { icon: Package, label: "Products", to: "/products" },
    { icon: BarChart3, label: "Analytics", to: "/analytics" },
    { icon: AlertTriangle, label: "Alerts", to: "/alerts" },
    { icon: Settings, label: "Settings", to: "/settings" }
    ...
    <div className="flex h-screen overflow-hidden bg-background text-foreground selection:bg-primary/30">
      ...
      >
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          ...
          >
            {profile?.photo ? (
              <img src={profile.photo} className="w-6 h-6 rounded-md object-cover shrink-0" alt="Profile" />
            ) : (
            ...
            {!collapsed && <span className="truncate">{profile?.fullName || 'My Profile'}</span>}
          </NavLink>
          <button
